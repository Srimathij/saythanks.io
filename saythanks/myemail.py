...
def notify(note, email_address):
    ...
    try:
        ...
        to_address = To(email_address)  # Use To() helper for newer versions of SendGrid (the email object for someone we’re sending to)
        content = Content('text/html', message)

        mail = Mail(from_email=from_address, to_emails=to_address, subject=subject, html_content=content)
